...
class ChatbotController extends BaseController
{
    public function handle(Request $request, Response $response): Response
    {
        ...
        if (count($_SESSION['chatbot_requests']) >= 20) {
            return $this->json($response, ['error' => 'Too many requests. Please wait a moment.'], 429);
        }
        ...
        if (empty($query) || strlen($query) < 2) {
            return $this->json($response, ['results' => [],'message' => 'Please type at least 2 characters.']);
        }

        // XSS protection. Prepared Statements been used.
        $blocked = [
            '/<script\b[^>]*>/i',          
            '/javascript\s*:/i',           
            '/on\w+\s*=\s*["\']?[^"\'>]*["\']?/i', 
            '/\b(exec|shell_exec|system|passthru)\b/i' 
        ];
        ...
        foreach ($blocked as $pattern) {
            if (preg_match($pattern, $query)) {
                ...
                return $this->json($response, [
                    'results' => [],
                    'message' => 'Security check: Please avoid using special characters or scripts.'
                ]);
            }
        }
        ...
        foreach ($pages as $page) {
            foreach ($page['keywords'] as $kw) {
                if (str_contains($lower, $kw)) {
                    ...
                    if (empty($already)) {
                        $results[] = ['type' => 'page','title' => $page['title'],'subtitle' => $page['subtitle'],'url' => $page['url'],'icon' => $page['icon']];
                    }
                    break;
                }
            }
