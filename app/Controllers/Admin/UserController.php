...
class UserController extends BaseController
{
    public function index(Request $request, Response $response): Response
    {
        ...
        // Massive JOIN to get all user stats in one go
        $stmt = $pdo->query('
            SELECT u.id, u.email, u.status, u.roles_mask, up.first_name, up.last_name, COUNT(o.id) as order_count 
            FROM users u 
            LEFT JOIN user_profiles up ON u.id = up.user_id 
            LEFT JOIN orders o ON u.id = o.user_id 
            GROUP BY u.id, u.email, u.status, u.roles_mask, up.first_name, up.last_name
        ');
        $users = $stmt->fetchAll();
    ...
    public function update(Request $request, Response $response): Response
    {
        ...
        if (isset($data['update_role'])) {
            ...
            $new_role = ($data['new_role'] ?? '') === 'admin' ? 'admin' : 'user';

            if ($uid === (int)$auth->getUserId() && $new_role !== 'admin') {
                $this->flash('You cannot remove your own admin privileges.', 'danger');
                return $this->redirect($response, '/admin/users');
            }
            ...
            try {
                ...
                $this->flash('Unknown user ID.', 'danger');
            }
        }
        ...
        if (isset($data['delete_user'])) {
            ...
            if ($uid === (int)$auth->getUserId()) {
                $this->flash('You cannot delete your own account.', 'danger');
                return $this->redirect($response, '/admin/users');
            }

            try {
                // Pre-clean data to prevent SQL constraint errors
                $pdo->prepare('DELETE FROM user_profiles WHERE user_id = :uid')->execute([':uid' => $uid]);
                $pdo->prepare('DELETE FROM user_sessions WHERE user_id = :uid')->execute([':uid' => $uid]);

                // Now delete the core user
                $auth->admin()->deleteUserById($uid);

                ekea_log('User deleted', 'WARNING', ['user_id' => $uid]);
                $this->flash('User account deleted.', 'success');
