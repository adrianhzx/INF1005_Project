...
class ProductController extends BaseController {
    ...
    public function index(Request $request, Response $response): Response {
        ...
        $where = []; $qparams = [];
        if ($category_filter > 0) { $where[] = 'p.category_id = :cat'; $qparams[':cat'] = $category_filter; }
        if ($search_query !== '') { $where[] = '(p.name LIKE :search OR p.description LIKE :search2)'; $qparams[':search'] = "%{$search_query}%"; $qparams[':search2'] = "%{$search_query}%"; }
        $where_sql = !empty($where) ? 'WHERE ' . implode(' AND ', $where) : '';

        $order = match($sort_by) {
            'price_asc' => 'p.price ASC', 'price_desc' => 'p.price DESC',
            'name' => 'p.name ASC', 'oldest' => 'p.created_at ASC',
        ...
        $page_title = 'Products'; $current_page = 'products';
        $csrf_token = generate_csrf_token();
        return $this->render($response, 'shop/products', compact('products','categories','total','total_pages','total_all','page','per_page','category_filter','search_query','sort_by','page_title','current_page','csrf_token'));
    }

    public function detail(Request $request, Response $response, array $args): Response {
        global $pdo, $auth;
        $product_id = (int)($args['id'] ?? 0);
        if ($product_id <= 0) { $this->flash('Invalid product.','danger'); return $this->redirect($response, '/products'); }
        ...
        foreach ($reviews as $rev) { $r=(int)$rev['rating']; if(isset($rating_dist[$r])) $rating_dist[$r]++; }

        $has_reviewed = false; $has_purchased = false;
        if ($auth->isLoggedIn()) {
        ...
        $page_title = $product['name']; $current_page = 'products'; $use_chartjs = true;
        $csrf_token = generate_csrf_token();
        return $this->render($response, 'shop/product_detail', compact('product','reviews','avg_rating','rating_dist','has_reviewed','has_purchased','related_products','product_id','page_title','current_page','use_chartjs','csrf_token'));
    }
    ...
    public function action(Request $request, Response $response, array $args): Response {
        ...
        if (isset($data['add_to_cart'])) {
            if (!$auth->isLoggedIn()) { $this->flash('Please log in to add items to your cart.','warning'); return $this->redirect($response, '/login'); }
            ...
            unset($item);
            if (!$found) $_SESSION['cart'][] = ['product_id'=>$product_id,'name'=>$product['name'],'price'=>$product['price'],'image_url'=>$product['image_url'],'quantity'=>$qty];
            $this->flash(htmlspecialchars($product['name'],ENT_QUOTES,'UTF-8').' added to your cart!','success');
        }

        // Submit review
        if (isset($data['submit_review'])) {
            if (!$auth->isLoggedIn()) { $this->flash('Please log in to leave a review.','warning'); return $this->redirect($response, '/login'); }
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM order_items oi JOIN orders o ON oi.order_id = o.id WHERE o.user_id = :uid AND oi.product_id = :pid AND o.status = "delivered"');
            $stmt->execute([':uid'=>$auth->getUserId(),':pid'=>$product_id]);
            if ($stmt->fetchColumn() == 0) { $this->flash('You must purchase and receive this product before leaving a review.','danger'); return $this->redirect($response, "/products/{$product_id}"); }
            $rating = max(1, min(5, (int)($data['rating'] ?? 0)));
            $comment = trim($data['comment'] ?? '');
            if ($rating < 1 || $rating > 5 || strlen($comment) < 10) { $this->flash('Please provide a valid rating and review of at least 10 characters.','danger'); return $this->redirect($response, "/products/{$product_id}"); }
            $pdo->prepare('INSERT INTO reviews (user_id, product_id, rating, comment) VALUES (:uid,:pid,:rating,:comment)')
                ->execute([':uid'=>$auth->getUserId(),':pid'=>$product_id,':rating'=>$rating,':comment'=>$comment]);
