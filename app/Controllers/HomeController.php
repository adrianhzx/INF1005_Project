...
class HomeController extends BaseController {
    ...
    public function news(Request $request, Response $response): Response {
        global $pdo;
        $stmt = $pdo->query('SELECT r.*, up.first_name, up.last_name, p.name AS product_name, p.id AS product_id, p.image_url FROM reviews r JOIN user_profiles up ON r.user_id = up.user_id JOIN products p ON r.product_id = p.id ORDER BY r.created_at DESC');
