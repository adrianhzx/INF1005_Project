    ...
    public function add(Request $request, Response $response): Response 
    {
        ...
        foreach ($_SESSION['cart'] as &$item) {
            if ($item['product_id'] === $product_id) { 
                // Stock validation check for existing cart items
                $new_total = $item['quantity'] + $qty;
                if ($new_total > $product['stock']) {
                    $item['quantity'] = $product['stock'];
                    $this->flash("You can only add up to {$product['stock']} of these to your cart.", 'warning');
                } else {
                    $item['quantity'] = $new_total;
                    $this->flash(htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8') . ' quantity updated in cart!', 'success');
                }
                $found = true; 
                break; 
            }
        ...
        if (!$found) {
            // Stock validation check for new cart items
            if ($qty > $product['stock']) {
                $qty = $product['stock'];
                $this->flash("Quantity adjusted to maximum available stock ({$product['stock']}).", 'warning');
            } else {
