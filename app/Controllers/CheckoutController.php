...
class CheckoutController extends BaseController
{
    public function index(Request $request, Response $response): Response
    {
        ...
        return $this->render($response, 'shop/checkout', compact('cart', 'subtotal', 'shipping', 'user_data', 'page_title', 'current_page', 'csrf_token', 'errors', 'stripe_pub_key'));
    }
    ...
    public function createIntent(Request $request, Response $response): Response
    {
        ...
        if (empty($stripe_secret_key)) {
            return $this->json($response, ['error' => 'Stripe is not configured on this server.']);
        }
        ...
        if ($result['status'] !== 200 || isset($result['data']['error'])) {
            ekea_log('Stripe PaymentIntent creation failed', 'ERROR', ['error' => $result['data']['error']['message'] ?? 'unknown']);
            return $this->json($response, ['error' => $result['data']['error']['message'] ?? 'Failed to initialise payment.']);
        }
        return $this->json($response, ['client_secret' => $result['data']['client_secret']]);
    }
    ...
    public function place(Request $request, Response $response): Response
    {
        ...
        if ($payment_method === 'credit_card' && empty($errors)) {
